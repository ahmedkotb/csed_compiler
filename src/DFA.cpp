...
NFA_State* DFA::create_DFA_state(vector<NFA_State*>* states) {
    ...
    // not complete
    // 1- if any of the NFA states is accepting , set DFA state as accepting
    // 2- set the accepting pattern of the new DFA state with the highest periority of the patterns of the NFA states
...
DFA* DFA::convert_NFA_to_DFA(NFA* nfa) {
    ...
    this->get_DFA(nfa->get_start_state()->get_transitions(EPSILON), nfa);
}
...
DFA* DFA::get_DFA(vector<NFA_State*>* states, NFA* nfa) {
    ...
    for (it = alphabet->begin(); it != alphabet->end(); it++) {
        // vector represeting group of NFA states (DFA State)
    ...
    for (int i = 0; i < row_new_states->size(); i++) {// loop through all DFA states in a table row and repeat the algorithm
